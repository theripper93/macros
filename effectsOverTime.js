...
//EffectOverTime false false @item.level 1 true false false false itemid actorid no **Example for spells with a save every turn**
//EffectOverTime true false auralevel 1 false true false true itemid actorid no **Example for aoes with damage every round**
...
for( let effect of effects){
   effect.changes[0].value = effect.changes[0].value.replace('itemid' , args[0].item._id).replace('actorid', args[0].actor._id).replace('auralevel' , args[0].item.data.level);
...
const damageType = DAEitem.data.damage.parts[0] ? DAEitem.data.damage.parts[0][1] : "";
...
if(proceed){
    ...
    let isSave = noSave ? false : await save(tactor, spellName,sAbility,"Repeating Saving Throw");
    if(isSave){
